...
`CommonMark <http://commonmark.org>`__, `PHP Markdown
Extra <https://michelf.ca/projects/php-markdown/extra/>`__,
`GitHub-Flavored
Markdown <https://help.github.com/articles/github-flavored-markdown/>`__,
and (subsets of) `Textile <http://redcloth.org/textile>`__,
`reStructuredText <http://docutils.sourceforge.net/docs/ref/rst/introduction.html>`__,
`HTML <http://www.w3.org/html/>`__,
`LaTeX <http://latex-project.org>`__, `MediaWiki
markup <https://www.mediawiki.org/wiki/Help:Formatting>`__, `TWiki
markup <http://twiki.org/cgi-bin/view/TWiki/TextFormattingRules>`__,
`Haddock
markup <https://www.haskell.org/haddock/doc/html/ch03s08.html>`__,
`OPML <http://dev.opml.org/spec2.html>`__, `Emacs Org
mode <http://orgmode.org>`__, `DocBook <http://docbook.org>`__,
`txt2tags <http://txt2tags.org>`__, `EPUB <http://idpf.org/epub>`__,
`ODT <http://en.wikipedia.org/wiki/OpenDocument>`__ and `Word
...
`HTML5 <http://www.w3.org/TR/html5/>`__,
`LaTeX <http://latex-project.org>`__ (including
```beamer`` <https://ctan.org/pkg/beamer>`__ slide shows),
`ConTeXt <http://contextgarden.net/>`__,
`RTF <http://en.wikipedia.org/wiki/Rich_Text_Format>`__,
`OPML <http://dev.opml.org/spec2.html>`__,
`DocBook <http://docbook.org>`__,
`OpenDocument <http://opendocument.xml.org>`__,
`ODT <http://en.wikipedia.org/wiki/OpenDocument>`__, `Word
docx <http://www.microsoft.com/interop/openup/openxml/default.aspx>`__,
`GNU Texinfo <http://www.gnu.org/software/texinfo/>`__, `MediaWiki
markup <https://www.mediawiki.org/wiki/Help:Formatting>`__, `DokuWiki
markup <https://www.dokuwiki.org/dokuwiki>`__, `Haddock
markup <https://www.haskell.org/haddock/doc/html/ch03s08.html>`__,
`EPUB <http://idpf.org/epub>`__ (v2 or v3),
`FictionBook2 <http://www.fictionbook.org/index.php/Eng:XML_Schema_Fictionbook_2.1>`__,
`Textile <http://redcloth.org/textile>`__, `groff
man <http://developer.apple.com/DOCUMENTATION/Darwin/Reference/ManPages/man7/groff_man.7.html>`__
pages, `Emacs Org mode <http://orgmode.org>`__,
`AsciiDoc <http://www.methods.co.nz/asciidoc/>`__, `InDesign
ICML <https://www.adobe.com/content/dam/Adobe/en/devnet/indesign/cs55-docs/IDML/idml-specification.pdf>`__,
and `Slidy <http://www.w3.org/Talks/Tools/Slidy/>`__,
`Slideous <http://goessner.net/articles/slideous/>`__,
`DZSlides <http://paulrouget.com/dzslides/>`__,
`reveal.js <http://lab.hakim.se/reveal-js/>`__ or
`S5 <http://meyerweb.com/eric/tools/s5/>`__ HTML slide shows. It can
also produce `PDF <https://www.adobe.com/pdf/>`__ output on systems
where LaTeX, ConTeXt, or ``wkhtmltopdf`` is installed.
...
elements <#extension-markdown_in_html_blocks>`__. (These enhancements,
described below under `Pandoc's Markdown <#pandocs-markdown>`__, can be
...
*stdout* is disabled for the ``odt``, ``docx``, ``epub``, and ``epub3``
output formats). For output to a file, use the ``-o`` option:
...
``-w/--write`` or ``-t/--to`` options. Thus, to convert ``hello.txt``
...
Note that the ``rst``, ``textile``, ``latex``, and ``html`` readers are
...
output through ```iconv`` <http://www.gnu.org/software/libiconv/>`__:
...
```eurosym`` <https://ctan.org/pkg/eurosym>`__,
...
```graphicx`` <https://ctan.org/pkg/graphicx>`__ and
```grffile`` <https://ctan.org/pkg/grffile>`__ (if the document contains
images), ```hyperref`` <https://ctan.org/pkg/hyperref>`__,
...
``lang``). The use of ``xelatex`` or ``lualatex`` as the LaTeX engine
requires ```fontspec`` <https://ctan.org/pkg/fontspec>`__; ``xelatex``
uses ```mathspec`` <https://ctan.org/pkg/mathspec>`__,
```polyglossia`` <https://ctan.org/pkg/polyglossia>`__ (with ``lang``),
```xecjk`` <https://ctan.org/pkg/xecjk>`__, and
```bidi`` <https://ctan.org/pkg/bidi>`__ (with the ``dir`` variable
set). The ```upquote`` <https://ctan.org/pkg/upquote>`__ and
...
used for [smart punctuation] if added to the template or included in any
header file. The ```natbib`` <https://ctan.org/pkg/natbib>`__,
...
used for [citation rendering]. These are included with all recent
versions of `TeX Live <http://www.tug.org/texlive/>`__.

Alternatively, pandoc can use ConTeXt or ``wkhtmltopdf`` to create a
PDF. To do this, specify an output file with a ``.pdf`` extension, as
before, but add ``-t context`` or ``-t html5`` to the command line.

PDF output can be controlled using [variables for LaTeX] (if LaTeX is
used) and [variables for ConTeXt] (if ConTeXt is used). If
``wkhtmltopdf`` is used, then the variables ``margin-left``,
...
``--read=``\ *FORMAT*
    ...
    ``markdown_github`` (GitHub-Flavored Markdown), ``commonmark``
    (CommonMark Markdown), ``textile`` (Textile), ``rst``
    (reStructuredText), ``html`` (HTML), ``docbook`` (DocBook), ``t2t``
    (txt2tags), ``docx`` (docx), ``odt`` (ODT), ``epub`` (EPUB),
    ``opml`` (OPML), ``org`` (Emacs Org mode), ``mediawiki`` (MediaWiki
    markup), ``twiki`` (TWiki markup), ``haddock`` (Haddock markup), or
    ``latex`` (LaTeX). If ``+lhs`` is appended to ``markdown``, ``rst``,
    ``latex``, or ``html``, the input will be treated as literate
    Haskell source: see `Literate Haskell
    support <#literate-haskell-support>`__, below. Markdown syntax
    extensions can be individually enabled or disabled by appending
    ``+EXTENSION`` or ``-EXTENSION`` to the format name. So, for
    ...
    Markdown <#pandocs-markdown>`__, below, for a list of extensions and
    their names.
...
``--write=``\ *FORMAT*
    ...
    Extra), ``markdown_github`` (GitHub-Flavored Markdown),
    ``commonmark`` (CommonMark Markdown), ``rst`` (reStructuredText),
    ``html`` (XHTML), ``html5`` (HTML5), ``latex`` (LaTeX), ``beamer``
    (LaTeX beamer slide show), ``context`` (ConTeXt), ``man`` (groff
    man), ``mediawiki`` (MediaWiki markup), ``dokuwiki`` (DokuWiki
    markup), ``textile`` (Textile), ``org`` (Emacs Org mode),
    ``texinfo`` (GNU Texinfo), ``opml`` (OPML), ``docbook`` (DocBook),
    ``opendocument`` (OpenDocument), ``odt`` (OpenOffice text document),
    ``docx`` (Word docx), ``haddock`` (Haddock markup), ``rtf`` (rich
    text format), ``epub`` (EPUB v2 book), ``epub3`` (EPUB v3), ``fb2``
    (FictionBook2 e-book), ``asciidoc`` (AsciiDoc), ``icml`` (InDesign
    ICML), ``slidy`` (Slidy HTML and javascript slide show),
    ...
    show), or the path of a custom lua writer (see `Custom
    writers <#custom-writers>`__, below). Note that ``odt``, ``epub``,
    and ``epub3`` output will not be directed to *stdout*; an output
    filename must be specified using the ``-o/--output`` option. If
    ``+lhs`` is appended to ``markdown``, ``rst``, ``latex``,
...
``-o`` *FILE*, ``--output=``\ *FILE*
    ...
    ``odt``, ``docx``, ``epub``, or ``epub3``, output to stdout is
    disabled.)
...
``-S``, ``--smart``
    ...
    automatically when the output format is ``latex`` or ``context``,
    unless ``--no-tex-ligatures`` is used. It has no effect for
    ``latex`` input.)
...
``--filter=``\ *EXECUTABLE*
    ...
    Note that the *EXECUTABLE* will be sought in the user's ``PATH``,
    and not in the working directory, if no directory is provided. If
...
``-M`` *KEY*\ [``=``\ *VAL*], ``--metadata=``\ *KEY*\ [``:``\ *VAL*]
    ...
    But unlike ``--variable``, ``--metadata`` affects the metadata of
    the underlying document (which is accessible from filters and may be
    printed in some output formats).

``--normalize``
    Normalize the document after reading: merge adjacent ``Str`` or
    ``Emph`` elements, for example, and remove repeated ``Space``\ s.
...
``--track-changes=accept``\ \|\ ``reject``\ \|\ ``all``
    Specifies what to do with insertions and deletions produced by the
    MS Word "Track Changes" feature. ``accept`` (the default), inserts
...
``-s``, ``--standalone``
    ...
    set automatically for ``pdf``, ``epub``, ``epub3``, ``fb2``,
    ``docx``, and ``odt`` output.

``--template=``\ *FILE*
    Use *FILE* as a custom template for the generated document. Implies
    ``--standalone``. See `Templates <#templates>`__, below, for a
...
``-V`` *KEY*\ [``=``\ *VAL*], ``--variable=``\ *KEY*\ [``:``\ *VAL*]
    ...
    the ``--template`` option is used to specify a custom template,
...
``-D`` *FORMAT*, ``--print-default-template=``\ *FORMAT*
    ...
    ``-t`` for a list of possible *FORMAT*\ s.) Templates in the user
...
``--columns=``\ *NUMBER*
    Specify length of lines in characters (for text wrapping). This
    affects only the generated source code, not the layout on the
    rendered page.

``--toc``, ``--table-of-contents``
    Include an automatically generated table of contents (or, in the
    case of ``latex``, ``context``, and ``rst``, an instruction to
    create one) in the output document. This option has no effect on
    ``man``, ``docbook``, ``slidy``, ``slideous``, ``s5``, ``docx``, or
    ``odt`` output.
...
``--highlight-style=``\ *STYLE*
    ...
    ``espresso``, ``zenburn``, ``haddock``, and ``tango``. For more
...
``--chapters``
    ...
    output. When the LaTeX document class is set to ``report``,
    ``book``, or ``memoir``, this option is implied. If ``beamer`` is
    the output format, top-level headers will become ``\part{..}``.
...
``--no-tex-ligatures``
    ...
    dashes (```...'``, ````..''``, ``--``, ``---``) when writing or
    reading LaTeX or ConTeXt. In reading LaTeX, parse the characters
    `````, ``'``, and ``-`` literally, rather than parsing ligatures for
    quotation marks and dashes. In writing LaTeX or ConTeXt, print
    ...
    selected. If you use literal curly quotes, dashes, and ellipses in
...
``--slide-level=``\ *NUMBER*
    ...
    slide. The default is to set the slide level based on the contents
    of the document; see `Structuring the slide
    show <#structuring-the-slide-show>`__.

``--section-divs``
    Wrap sections in ``<div>`` tags (or ``<section>`` tags in HTML5),
    and attach identifiers to the enclosing ``<div>`` (or ``<section>``)
    rather than the header itself. See `Header
    identifiers <#header-identifiers>`__, below.

``--email-obfuscation=none``\ \|\ ``javascript``\ \|\ ``references``
    Specify a method for obfuscating ``mailto:`` links in HTML
    documents. ``none`` leaves ``mailto:`` links as they are.
    ``javascript`` obfuscates them using javascript. ``references``
...
``--reference-docx=``\ *FILE*
    ...
    user data directory (see ``--data-dir``). If this is not found
    either, sensible defaults will be used. The following styles are
    ...
    Table Caption, Image Caption, Figure, Figure With Caption, TOC
    Heading; [character] Default Paragraph Font, Body Text Char,
    Verbatim Char, Footnote Reference, Hyperlink; [table] Normal Table.
...
``--epub-metadata=``\ *FILE*
    ...
    elements <http://dublincore.org/documents/dces/>`__. For example:
    ...
    format <http://www.w3.org/TR/NOTE-datetime>`__), ``<dc:language>``
...
``--natbib``
    ...
    ``pandoc-citeproc`` filter or with PDF output. It is intended for
    use in producing a LaTeX file that can be processed with
    ```bibtex`` <https://ctan.org/pkg/bibtex>`__.

``--biblatex``
    Use ```biblatex`` <https://ctan.org/pkg/biblatex>`__ for citations
...
``-m`` [*URL*\ ], ``--latexmathml``\ [``=``\ *URL*]
    Use the `LaTeXMathML <http://math.etsu.edu/LaTeXMathML/>`__ script
    to display embedded TeX math in HTML output. To insert a link to a
...
``--mathml``\ [``=``\ *URL*]
    Convert TeX math to `MathML <http://www.w3.org/Math/>`__ (in
    ``docbook`` as well as ``html`` and ``html5``). In standalone
    ``html`` output, a small javascript (or a link to such a script if a
    *URL* is supplied) will be inserted that allows the MathML to be
    viewed on some browsers.

``--jsmath``\ [``=``\ *URL*]
    Use `jsMath <http://www.math.union.edu/~dpvc/jsmath/>`__ to display
...
``--mathjax``\ [``=``\ *URL*]
    ...
    math in HTML output. The *URL* should point to the ``MathJax.js``
...
``--gladtex``
    ...
    processed by `gladTeX <http://ans.hsh.no/home/mgg/gladtex/>`__ to
    produce links to images of the typeset formulas.

``--mimetex``\ [``=``\ *URL*]
    Render TeX math using the
    `mimeTeX <http://www.forkosh.com/mimetex.html>`__ CGI script. If
...
``--katex``\ [``=``\ *URL*]
    Use `KaTeX <https://github.com/Khan/KaTeX>`__ to display embedded
    TeX math in HTML output. The *URL* should point to the ``katex.js``
    load script. If a *URL* is not provided, a link to the KaTeX CDN
    will be inserted.

``--katex-stylesheet=``\ *URL*
    The *URL* should point to the ``katex.css`` stylesheet. If this
    option is not specified, a link to the KaTeX CDN will be inserted.
...
within the document using `YAML metadata
blocks <#extension-yaml_metadata_block>`__. They may also be set at the
...
``subtitle``
    ...
    `KOMA-Script <https://ctan.org/pkg/koma-script>`__ series
...
``otherlangs``
    ...
    according to `BCP 47 <https://tools.ietf.org/html/bcp47>`__. For
    example: ``otherlangs: [en-GB, fr]``. This is automatically
    generated from the ``lang`` attributes in all ``span``\ s and
    ``div``\ s but can be overriden. Currently only used by LaTeX
    through the generated ``babel-otherlangs`` and
    ``polyglossia-otherlangs`` variables. The LaTeX writer outputs
    polyglossia commands in the text but the ``babel-newcommands``
...
``dir``
    ...
    be used to override the base direction in some output formats. This
    may not always be necessary if the final renderer (e.g. the browser,
...
options <https://github.com/hakimel/reveal.js#configuration>`__.
...
``theme``, ``colortheme``, ``fonttheme``, ``innertheme``, ``outertheme``
    themes for LaTeX ```beamer`` <https://ctan.org/pkg/beamer>`__
    documents
``navigation``
...
``documentclass``
    document class, e.g. ```article`` <https://ctan.org/pkg/article>`__,
    ```report`` <https://ctan.org/pkg/report>`__,
    ```book`` <https://ctan.org/pkg/book>`__,
    ```memoir`` <https://ctan.org/pkg/memoir>`__
``classoption``
    option for document class, e.g. ``oneside``; may be repeated for
    multiple options
``geometry``
    option for ```geometry`` <https://ctan.org/pkg/geometry>`__ package,
    e.g. ``margin=1in``; may be repeated for multiple options
``margin-left``, ``margin-right``, ``margin-top``, ``margin-bottom``
    sets margins, if ``geometry`` is not used (otherwise ``geometry``
    overrides these)
``linestretch``
    adjusts line spacing using the
    ```setspace`` <https://ctan.org/pkg/setspace>`__ package, e.g.
    ``1.25``, ``1.5``
``fontfamily``
    font package for use with ``pdflatex``: `TeX
    Live <http://www.tug.org/texlive/>`__ includes many options,
    documented in the `LaTeX Font
    Catalogue <http://www.tug.dk/FontCatalogue/>`__. The default is
    `Latin Modern <https://ctan.org/pkg/lm>`__.
``fontfamilyoptions``
    options for package used as ``fontfamily``: e.g. ``osf,sc`` with
    ``fontfamily`` set to
    ```mathpazo`` <https://ctan.org/pkg/mathpazo>`__ provides Palatino
    with old-style figures and true small caps; may be repeated for
    multiple options
``mainfont``, ``sansfont``, ``monofont``, ``mathfont``, ``CJKmainfont``
    font families for use with ``xelatex`` or ``lualatex``: take the
    name of any system font, using the
    ```fontspec`` <https://ctan.org/pkg/fontspec>`__ package. Note that
    if ``CJKmainfont`` is used, the
    ```xecjk`` <https://ctan.org/pkg/xecjk>`__ package must be
    available.
...
``mathfontoptions``, ``CJKoptions``
    ...
    ```fontspec`` <https://ctan.org/pkg/fontspec>`__, such as the
...
``fontenc``
    ...
    encodings <https://ctan.org/pkg/encguide>`__)
``colorlinks``
...
``linkcolor``, ``citecolor``, ``urlcolor``, ``toccolor``
    ...
    colors <https://en.wikibooks.org/wiki/LaTeX/Colors#Predefined_colors>`__
``links-as-notes``
...
``papersize``
    ...
    Paper Setup <http://wiki.contextgarden.net/PaperSetup>`__); may be
    repeated for multiple options
``layout``
    options for page margins and text arrangement (see `ConTeXt
    Layout <http://wiki.contextgarden.net/Layout>`__); may be repeated
...
``mainfont``, ``sansfont``, ``monofont``, ``mathfont``
    ...
    Switching <http://wiki.contextgarden.net/Font_Switching>`__)
``linkcolor``, ``contrastcolor``
    color for links outside and inside a page, e.g. ``red``, ``blue``
    (see `ConTeXt Color <http://wiki.contextgarden.net/Color>`__)
``linkstyle``
...
``indenting``
    ...
    `ConTeXt
    Indentation <http://wiki.contextgarden.net/Indentation>`__); may be
    repeated for multiple options
``whitespace``
    spacing between paragraphs, e.g. ``none``, ``small`` (using
    ```setupwhitespace`` <http://wiki.contextgarden.net/Command/setupwhitespace>`__)
``interlinespace``
    adjusts line spacing, e.g. ``4ex`` (using
    ```setupinterlinespace`` <http://wiki.contextgarden.net/Command/setupinterlinespace>`__);
    may be repeated for multiple options
``headertext``, ``footertext``
    text to be placed in running header or footer (see `ConTeXt Headers
    and Footers <http://wiki.contextgarden.net/Headers_and_Footers>`__);
    may be repeated up to four times for different placement
``pagenumbering``
    page number style and location (using
    ```setuppagenumbering`` <http://wiki.contextgarden.net/Command/setuppagenumbering>`__);
    may be repeated for multiple options
``toc``
    include table of contents (can also be set using
    ``--toc/--table-of-contents``)
...
starting with a letter. A variable name surrounded by ``$`` signs will
...
value; otherwise it will include ``Y``. ``X`` and ``Y`` are placeholders
for any valid template text, and may include interpolated variables or
...
Gruber's `Markdown <http://daringfireball.net/projects/markdown/>`__
...
``markdown``. An extensions can be enabled by adding ``+EXTENSION`` to
the format name and disabled by adding ``-EXTENSION``. For example,
...
wrapped in a ``div`` (or a ``section``, if ``--html5`` was specified),
and the identifier will be attached to the enclosing ``<div>`` (or
``<section>``) tag rather than the header itself. This allows entire
...
and LaTeX. If highlighting is supported for your output format and
language, then the code block above will appear highlighted, with
numbered lines. (To see which languages are supported, do
``pandoc --version``.) Otherwise, the code block above will appear as
follows:
...
guide <http://daringfireball.net/projects/markdown/syntax#list>`__, the
...
The `Markdown syntax
guide <http://daringfireball.net/projects/markdown/syntax#list>`__ is
not explicit whether the four-space rule applies to *all* block-level
...
mode <http://table.sourceforge.net/>`__.
...
have an optional prefix attached (``--title-prefix`` or ``-T`` option).
...
```csquotes`` <https://ctan.org/pkg/csquotes>`__ package, pandoc will
...
HTML, Slidy, DZSlides, S5, EPUB
    ...
    1. The default is to render TeX math as far as possible using
       ...
       output. Formulas are put inside a ``span`` with ``class="math"``,
       so that they may be styled differently from the surrounding text
       if needed.

    2. If the ``--latexmathml`` option is used, TeX math will be
       displayed between ``$`` or ``$$`` characters and put in
       ``<span>`` tags with class ``LaTeX``. The
       `LaTeXMathML <http://math.etsu.edu/LaTeXMathML/>`__ script will
       be used to render it as formulas. (This trick does not work in
       all browsers, but it works in Firefox. In browsers that do not
    ...
    3. If the ``--jsmath`` option is used, TeX math will be put inside
       ...
       `jsMath <http://www.math.union.edu/~dpvc/jsmath/>`__ script will
       be used to render it.

    4. If the ``--mimetex`` option is used, the
       `mimeTeX <http://www.forkosh.com/mimetex.html>`__ CGI script will
    ...
    5. If the ``--gladtex`` option is used, TeX formulas will be
       enclosed in ``<eq>`` tags in the HTML output. The resulting
       ``htex`` file may then be processed by
       `gladTeX <http://ans.hsh.no/home/mgg/gladtex/>`__, which will
    ...
    7. If the ``--mathjax`` option is used, TeX math will be displayed
       ...
       `MathJax <https://www.mathjax.org>`__ script will be used to
       render it as formulas.
...
``\newcommand`` and ``\renewcommand`` definitions and apply the
...
generated identifier (see `Header identifiers <#header-identifiers>`__).
For example:
...
with a caption. [5]_ (In LaTeX, a figure environment will be used; in
HTML, the image will be placed in a ``div`` with class ``figure``,
together with a caption in a ``p`` with class ``caption``.) The image's
alt text will be used as the caption.
...
-  Some output formats have a notion of a class
   (`ConTeXt <http://wiki.contextgarden.net/Using_Graphics#Multiple_Image_Settings>`__)
...
the `Citation Style Language <http://citationstyles.org>`__, listed in
...
Style <http://chicagomanualofstyle.org>`__ author-date format. The CSL
project provides further information on `finding and editing
styles <http://citationstyles.org/styles/>`__.
...
```biblatex`` <https://ctan.org/pkg/biblatex>`__ to render bibliography.
In order to do so, specify bibliography files as outlined above, and add
``--natbib`` or ``--biblatex`` argument to ``pandoc`` invocation. Bear
in mind that bibliography files have to be in respective format (either
BibTeX or BibLaTeX).
...
Enables a `MultiMarkdown <http://fletcherpenney.net/multimarkdown/>`__
...
-  Lazy wrapping of paragraphs is not allowed: the entire definition
   must be indented four spaces. [6]_
...
``markdown_phpextra`` (PHP Markdown Extra)
    ``footnotes``, ``pipe_tables``, ``raw_html``,
    ``markdown_attribute``, ``fenced_code_blocks``,
    ``definition_lists``, ``intraword_underscores``,
...
``markdown_mmd`` (MultiMarkdown)
    ...
    ``mmd_link_attributes``, ``raw_tex``, ``tex_math_double_backslash``,
...
-  ``auto_identifiers`` can be used with ``latex``, ``rst``,
   ``mediawiki``, and ``textile`` input (and is used by default).

-  ``tex_math_dollars``, ``tex_math_single_backslash``, and
   ``tex_math_double_backslash`` can be used with ``html`` input. (This
   is handy for reading web pages formatted using MathJax, for example.)
...
using `S5 <http://meyerweb.com/eric/tools/s5/>`__,
`DZSlides <http://paulrouget.com/dzslides/>`__,
`Slidy <http://www.w3.org/Talks/Tools/Slidy/>`__,
`Slideous <http://goessner.net/articles/slideous/>`__, or
`reveal.js <http://lab.hakim.se/reveal-js/>`__. You can also produce a
PDF slide show using LaTeX ```beamer`` <https://ctan.org/pkg/beamer>`__.
...
the ``-s/--standalone`` option embeds a link to javascripts and CSS
...
``revealjs-url``, or ``s5-url`` variables; see `Variables for
slides <#variables-for-slides>`__, above.) For DZSlides, the (relatively
...
`Beamer User's
Guide <http://ctan.math.utah.edu/ctan/tex-archive/macros/latex/contrib/beamer/doc/beameruserguide.pdf>`__
...
``creator``
    ...
    for ``role`` are `MARC
    relators <http://loc.gov/marc/relators/relaterm.html>`__, but pandoc
...
instead, use raw HTML in your source and add ``data-external="1"`` to
...
If you append ``+lhs`` (or ``+literate_haskell``) to an appropriate
input or output format (``markdown``, ``markdown_strict``, ``rst``, or
``latex`` for input or output; ``beamer``, ``html`` or ``html5`` for
...
`highlighting-kate <https://github.com/jgm/highlighting-kate>`__ is used
for highlighting, which works in HTML, Docx, and LaTeX/PDF output. The
color scheme can be selected using the ``--highlight-style`` option. The
...
`lua <http://www.lua.org>`__. (Pandoc includes a lua interpreter, so lua
...
`GPL <http://www.gnu.org/copyleft/gpl.html>`__, version 2 or greater.
...
.. [5]
   ...
   those formats, you'll just get an image in a paragraph by itself,
   with no caption.
